"""
Large-scale Cross-lingual Training Script (v6)

Changes from v5:
- Uses cleaned dataset (no Cyrillic/noise)
- Uses soft_target expansion loss (more stable than additive)
- Added Cyrillic single chars to noise tokens
"""
...
class TrainingConfig:
    ...
    lambda_noise: float = 0.5  # Increased from 0.3

    # Loss types - CHANGED from additive to soft_target
    expansion_loss_type: str = "soft_target"
    expansion_top_k: int = None  # Not used for soft_target
    kd_loss_type: str = "relation"
    noise_penalty_type: str = "sum"

    # Noise tokens - Extended with Cyrillic single chars
    noise_tokens: tuple = (
        ...
        # Cyrillic single characters (prevent collapse)
        "Д", "Т", "Н", "Г", "П", "В", "Ч", "С", "М", "К",
        "д", "т", "н", "г", "п", "в", "ч", "с", "м", "к",
        "А", "О", "Е", "И", "У", "Р", "Л", "Б", "Ф", "Я",
    )
    ...
    # Output
    output_dir: str = "outputs/cross_lingual_expansion_v6_largescale"
    log_steps: int = 100
    save_steps: int = 5000
    eval_steps: int = 5000  # More frequent evaluation
...
def train_step(
    ...
    # 1. Token expansion loss (soft_target: KL divergence)
...
def evaluate(
    ...
    for ko_term, en_synonyms in TEST_PAIRS:
        ...
        detailed_results.append({
            'ko': ko_term,
            'expected': en_synonyms,
            'activated': activated_en,
            'top10': list(top_k_tokens)[:10],
        })

    model.train()
    return {
        'activation_rate': total_activated / total_expected if total_expected > 0 else 0,
...
def main():
    ...
    # Training loop
    print(f"\n{'='*60}")
    print(f"STARTING LARGE-SCALE TRAINING (v6)")
    print(f"{'='*60}")
    print(f"Total steps: {total_steps:,}")
    print(f"Warmup steps: {warmup_steps:,}")
    print(f"Expansion loss type: {config.expansion_loss_type}")
    ...
    print(f"\nInitial activation rate: {eval_results['activation_rate']:.1%}")
        ...
        for step, batch in enumerate(pbar):
            ...
            if (step + 1) % config.gradient_accumulation_steps == 0:
                ...
                if global_step % config.eval_steps == 0:
                    ...
                    for detail in eval_results['details'][:3]:
                        print(f"  {detail['ko']}: {detail['activated']} | Top-10: {detail['top10'][:5]}...")

#!/usr/bin/env python3
"""
v14 Training Script: Augmented KO-EN Neural Sparse Model

Key improvements from v13:
1. Augmented training data with missing key terms (recommend, natural, neural, etc.)
2. Added abbreviation mappings (AI, ML, NLP, etc.)
3. Added compound term decomposition
4. Increased oversampling of important terms
"""
...
CONFIG = {
    ...
    'max_length': 64,
    'data_path': PROJECT_ROOT / 'dataset' / 'v14_augmented' / 'term_pairs.jsonl',
    'batch_size': 128,
    'num_epochs': 10,
    'learning_rate': 2e-5,
    'warmup_ratio': 0.1,
    ...
    'target_margin': 2.0,
    'output_dir': PROJECT_ROOT / 'outputs' / 'v14_augmented',
}
    ...
    def __init__(self, data_path: Path, tokenizer, max_length: int = 64):
        ...
        with open(data_path, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc="Loading data"):
                ...
                ko_term = item.get('ko_text') or item.get('ko', '')
                en_term = item.get('en_text') or item.get('en', '')
...
# Extended test pairs with previously missing terms
TEST_PAIRS = [
    # Original tests
    ...
    # New tests for previously missing terms
...
def main():
    print("=" * 70)
    print("v14 TRAINING: Augmented KO-EN Neural Sparse Model")
    print("=" * 70)

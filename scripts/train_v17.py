#!/usr/bin/env python3
"""
v17 Training Script: XLM-RoBERTa-large with Adjusted Hyperparameters

Key changes from v16:
1. Learning rate: 2e-6 (5x lower than v16's 1e-5)
2. Warmup ratio: 20% (2x higher than v16's 10%)
3. Use high-quality v15 curated dataset (not external data)
4. More epochs since dataset is smaller
"""
...
# Configuration - adjusted for stable training
CONFIG = {
    # Model - XLM-RoBERTa-large
    "model_name": "xlm-roberta-large",
    "max_length": 64,

    # Data - use high-quality v15 curated dataset
    "data_path": PROJECT_ROOT / "dataset" / "v15_aggressive" / "term_pairs.jsonl",

    # Training - conservative hyperparameters for large model
    "batch_size": 32,
    "gradient_accumulation_steps": 4,  # Effective batch size: 128
    "num_epochs": 10,  # More epochs for smaller dataset
    "learning_rate": 2e-6,  # 5x lower than v16
    "warmup_ratio": 0.2,  # 2x higher than v16
    "max_grad_norm": 1.0,

    # Loss weights - adjusted for better balance
    "lambda_self": 2.0,  # Increased for Korean preservation
    "lambda_target": 5.0,
    "lambda_margin": 3.0,
    "lambda_negative": 0.5,
    "lambda_sparsity": 0.005,  # Reduced to allow more activations
    "target_margin": 2.0,
    ...
    # Output
    "output_dir": PROJECT_ROOT / "outputs" / "v17_xlm_large",
}
...
def main():
    print("=" * 70)
    print("v17 TRAINING: XLM-RoBERTa-large with Adjusted Hyperparameters")
    print("=" * 70)
    print("\nKey changes from v16:")
    print(f"  - Learning rate: 2e-6 (was 1e-5)")
    print(f"  - Warmup ratio: 20% (was 10%)")
    print(f"  - Dataset: v15 curated (was v16 external)")
    print(f"  - Self loss weight: 2.0 (was 1.0)")
    print(f"  - Sparsity loss: 0.005 (was 0.01)")
    ...
    # Training
    history = []
    best_score = 0  # Combined score: ko_rate + en_rate
    global_step = 0

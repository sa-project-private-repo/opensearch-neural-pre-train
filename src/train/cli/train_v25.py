"""
CLI for V25 XLM-RoBERTa training with IDF-Aware FLOPS.

Key difference from V24: IDF weights are computed from corpus and
passed to SPLADELossV25, ensuring proper frequency-based discrimination
for the 250K XLM-RoBERTa vocabulary.

Usage:
    python -m train v25
    python -m train v25 --config configs/train_v25.yaml
    python -m train v25 --resume
"""
...
from src.train.config.v25 import V25Config, create_default_v25_config
...
from src.train.idf import load_or_compute_idf, create_stopword_mask
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="V25 XLM-RoBERTa Training with IDF-Aware FLOPS",
...
def build_config(args: argparse.Namespace) -> V25Config:
    """
    ...
    # Load config
    if args.config:
        config = load_config(args.config, V25Config, overrides)
    else:
        # Create default config
        train_files = args.train_files
        val_files = args.val_files

        config = create_default_v25_config(
            train_files=train_files,
            val_files=val_files,
            output_dir=args.output_dir or "outputs/train_v25",
...
def create_xlmr_model(config: V25Config) -> torch.nn.Module:
    """
    Create XLM-RoBERTa SPLADE model.

    Args:
        config: V25 configuration
...
def setup_training(
    ...
    """
    Setup training components for V25.

    Key difference from V24: This function loads or computes IDF weights
    from the training corpus and creates SPLADELossV25 with proper
    ...
    # === V25 SPECIFIC: Compute or load IDF weights ===
    ...
    # === V25 SPECIFIC: Create stopword mask ===
    stopword_mask = None
    if config.loss.use_stopword_mask:
        logger.info("Creating Korean stopword mask...")
        stopword_mask = create_stopword_mask(tokenizer)
        masked_count = (stopword_mask == 0).sum().item()
        logger.info(f"Stopword mask: {masked_count} tokens masked")

    # Create V25 loss function with mandatory IDF weights
    logger.info("Creating SPLADELossV25 with IDF-aware FLOPS...")
...
def main() -> int:
    """Main entry point for V25 training."""
    args = parse_args()
    ...
    logger.info("=" * 70)
    logger.info("SPLADE V25 XLM-RoBERTa Training with IDF-Aware FLOPS")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info("V25 IDF Configuration:")

"""
CLI for V27 XLM-RoBERTa training with Travel Domain Data.

V27 extends V26 with travel/tourism domain data to improve
location-specific query understanding.

Key differences from V26:
- Includes travel domain training data (~60K triplets)
- Better coverage for travel queries (서울 여행 추천)
- Location-aware hard negatives (Seoul vs Busan)

Usage:
    python -m src.train v27
    python -m src.train v27 --config configs/train_v27.yaml
    python -m src.train v27 --resume
"""
...
# V27 default data paths (V26 + travel domain)
V27_DEFAULT_TRAIN_FILES = [
    "data/v24.0/train_*.jsonl",
    "data/v27.0/train_travel_*.jsonl",
]
V27_DEFAULT_VAL_FILES = [
    "data/v24.0/val.jsonl",
    "data/v27.0/val_travel.jsonl",
]
V27_DEFAULT_OUTPUT_DIR = "outputs/train_v27"


def parse_args() -> argparse.Namespace:
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="V27 XLM-RoBERTa Training with Travel Domain Data",
    ...
    # V26/V27 specific options
    parser.add_argument(
        "--special-penalty",
        type=float,
        default=None,
        help="Fixed penalty for special tokens (default: 100.0)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Stopword penalty multiplier (default: 15.0)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="FLOPS regularization weight (default: 0.010)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="IDF curve sharpness (default: 4.0)",
    )
...
def build_config(args: argparse.Namespace) -> V26Config:
    ...
    Uses V26Config since V27 uses the same model and loss,
    ...
    Returns:
        V26Config instance with V27 defaults
    """
    ...
    if args.config:
        ...
        # Create default config with V27 data paths
        train_files = args.train_files or V27_DEFAULT_TRAIN_FILES
        val_files = args.val_files or V27_DEFAULT_VAL_FILES

        config = create_default_v26_config(
            train_files=train_files,
            val_files=val_files,
            output_dir=args.output_dir or V27_DEFAULT_OUTPUT_DIR,
        ...
        # Update experiment name for V27
        config.training.experiment_name = "splade_v27_xlmr_travel_domain"
...
def setup_training(
    ...
    """
    Setup training components for V27.

    Uses V26's enhanced IDF handling with travel domain data.

    Args:
        config: V26 configuration (with V27 data paths)
    ...
    # Get special token IDs (V26 feature)
    ...
    # Create extended stopword mask (V26 feature)
    stopword_mask = None
    ...
    # Create V26 loss function (used by V27)
    logger.info("Creating SPLADELossV26 for V27 training...")
    ...
    # Load training data (V27: includes travel domain)
    ...
    # Check for travel domain data
    travel_count = sum(c for t, c in pair_counts.items() if "travel" in t.lower())
    if travel_count > 0:
        logger.info(f"Travel domain samples: {travel_count:,}")
    else:
        logger.warning("No travel domain data found! Run: make prepare-v27-data")
...
def main() -> int:
    """Main entry point for V27 training."""
    args = parse_args()
    ...
    logger.info("=" * 70)
    logger.info("SPLADE V27 XLM-RoBERTa Training with Travel Domain Data")
    logger.info("=" * 70)
    logger.info("V27 improvements over V26:")
    logger.info("  - Travel/tourism domain data (~60K triplets)")
    logger.info("  - Better location token activation (서울, 부산, 제주)")
    logger.info("  - Location-aware hard negatives")
    logger.info("  - Improved travel query understanding")
    logger.info("=" * 70)
    ...
    try:
        ...
        for f in config.data.val_files:
            logger.info(f"  - {f}")
        ...
        logger.info("  1. Convert to HuggingFace: make convert-v27-hf")
        logger.info("  2. Evaluate travel queries: make eval-v27-travel")
        logger.info("=" * 70)
        ...
        logger.error("Did you run: make prepare-v27-data ?")
        return 1

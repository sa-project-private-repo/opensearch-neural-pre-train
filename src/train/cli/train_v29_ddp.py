"""
V29 Multi-GPU Training with DistributedDataParallel (DDP).

V29 combines SPLADE v2 improvements with AI Hub data:
- Max pooling (SPLADE v2 Eq. 6)
- Separate FLOPS regularization (lambda_q, lambda_d)
- Quadratic lambda warmup schedule
- AI Hub Korean data integration
...
Usage:
    ...
    # With custom config
    torchrun --nproc_per_node=8 -m src.train.cli.train_v29_ddp --epochs 20 --batch-size 32
...
from src.train.config.v29 import V29Config, V29LossConfig, V29ModelConfig
...
from src.train.schedulers import QuadraticLambdaScheduler
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="V29 Multi-GPU Training with DDP (SPLADE v2 + AI Hub)",
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/train_v29_ddp",
                        help="Output directory")
    parser.add_argument("--resume", action="store_true", help="Resume from latest checkpoint")
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Specific checkpoint to resume from")

    # V29 specific - SPLADE v2 FLOPS parameters
    parser.add_argument("--pooling", type=str, default="sum", choices=["max", "sum"],
                        help="Pooling strategy (sum=stable, max=SPLADE v2 requires distillation)")
    parser.add_argument("--lambda-flops-q", type=float, default=1e-4,
                        help="Query FLOPS regularization weight")
    parser.add_argument("--lambda-flops-d", type=float, default=1e-3,
                        help="Document FLOPS regularization weight")
    parser.add_argument("--lambda-warmup-steps", type=int, default=15000,
                        help="Steps for quadratic lambda warmup (should be < total steps)")

    # V28 inherited - Language filtering (reduced penalties)
    ...
    parser.add_argument("--no-context-gate", action="store_true",
                        help="Disable context gate")
    parser.add_argument("--no-language-filtering", action="store_true",
...
def create_v29_config(args: argparse.Namespace) -> V29Config:
    """Create V29 config with SPLADE v2 improvements."""
    use_context_gate = not args.no_context_gate

    # Select model class based on pooling and context gate
    if use_context_gate:
    ...
    config = V29Config(
        ...
        data=DataConfig(
            ...
            max_length=192,  # Reduced from 256 to match V28 memory usage
            num_workers=4,
        ),
        loss=V29LossConfig(
            # V29: Separate FLOPS weights
            lambda_flops_q=args.lambda_flops_q,
            lambda_flops_d=args.lambda_flops_d,
            lambda_warmup_steps=args.lambda_warmup_steps,
            lambda_flops=0.0,  # Disabled, use lambda_flops_q/d

            # V28: Language filtering (reduced penalties)
        ...
        training=TrainingConfig(
            ...
            experiment_name="splade_v29_ddp_max_pooling",
...
def create_model(config: V29Config, device: torch.device) -> nn.Module:
    """Create V29 model with configurable pooling."""
    ...
    if model_class == "SPLADEDocV29ContextGated":
        ...
        logger.info(f"Created SPLADEDocV29ContextGated model (pooling={pooling})")
    elif model_class == "SPLADEDocV29":
        model = SPLADEDocV29(
            model_name=config.model.name,
            dropout=config.model.dropout,
            use_mlm_head=config.model.use_expansion,
        ...
        logger.info("Created SPLADEDocContextGated model (V28 style)")
    else:
        model = SPLADEDocXLMR(
            model_name=config.model.name,
            dropout=config.model.dropout,
...
def create_loss_fn(config: V29Config, tokenizer, device: torch.device) -> nn.Module:
    """Create V29 loss function with separate FLOPS weights."""
    ...
    loss_fn = SPLADELossV29(
        ...
        # V29 specific
        lambda_flops_q=config.loss.lambda_flops_q,
        lambda_flops_d=config.loss.lambda_flops_d,
        use_margin_mse=config.loss.use_margin_mse,
        # V28 inherited
...
def train_epoch(
    ...
    optimizer: AdamW,
    scheduler,
    lambda_scheduler: QuadraticLambdaScheduler,
    config: V29Config,
    epoch: int,
    global_step: int,
    device: torch.device,
    ...
    """Train one epoch with DDP and lambda warmup."""
    model.train()
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        with autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            # Compute loss - use contiguous copies of input_ids
        ...
        if (batch_idx + 1) % config.training.gradient_accumulation_steps == 0:
            ...
            # Update lambda scheduler (V29 quadratic warmup)
            lambda_q, lambda_d = lambda_scheduler.get_lambda(global_step)
            loss_fn.update_lambda_flops(lambda_q, lambda_d)
            ...
            if is_main_process() and global_step % config.training.log_every_n_steps == 0:
                ...
                semantic_ratio = (loss_fn.get_average_semantic_ratio()
                                  if hasattr(loss_fn, 'get_average_semantic_ratio') else 0)
                korean_ratio = (loss_fn.get_average_korean_ratio()
                                if hasattr(loss_fn, 'get_average_korean_ratio') else 0)

                logger.info(
                    f"Step {global_step} - loss: "
                    f"{loss.item() * config.training.gradient_accumulation_steps:.4f}, "
                    f"lr: {lr:.2e}, lambda_q: {lambda_q:.2e}, lambda_d: {lambda_d:.2e}, "
                    f"semantic_ratio: {semantic_ratio:.4f}, korean_ratio: {korean_ratio:.4f}"
                )
                ...
                if tb_logger:
                    ...
                    tb_logger.log_scalar("train/lambda_q", lambda_q, global_step)
                    tb_logger.log_scalar("train/lambda_d", lambda_d, global_step)
                    ...
                    if "flops_q" in loss_dict:
                        tb_logger.log_scalar("train/flops_q", loss_dict["flops_q"], global_step)
                        tb_logger.log_scalar("train/flops_d", loss_dict["flops_d"], global_step)
...
def save_checkpoint(
    ...
    torch.save(lambda_scheduler.state_dict(), checkpoint_dir / "lambda_scheduler.pt")
...
def main():
    ...
    if is_main_process():
        ...
        logger.info("=" * 70)
        logger.info("V29 Multi-GPU Training with DDP (SPLADE v2 + AI Hub)")
        logger.info("=" * 70)
        logger.info(f"World size: {dist.get_world_size()}")
        logger.info(f"Local rank: {local_rank}")
        logger.info("")
        logger.info("=== V29 Features ===")
        logger.info(f"  Pooling: {args.pooling}")
        logger.info(f"  lambda_flops_q: {args.lambda_flops_q}")
        logger.info(f"  lambda_flops_d: {args.lambda_flops_d}")
        logger.info(f"  lambda_warmup_steps: {args.lambda_warmup_steps}")
    ...
    # Create V29 lambda scheduler (quadratic warmup)
    lambda_scheduler = QuadraticLambdaScheduler(
        initial_lambda_q=0.0,
        initial_lambda_d=0.0,
        target_lambda_q=config.loss.lambda_flops_q,
        target_lambda_d=config.loss.lambda_flops_d,
        warmup_steps=config.loss.lambda_warmup_steps,
    )

    if is_main_process():
        logger.info(f"Total steps: {total_steps}, LR warmup steps: {warmup_steps}")
        logger.info(f"Lambda warmup steps: {config.loss.lambda_warmup_steps}")
    ...
    if args.resume or args.checkpoint:
        ...
        if checkpoint_path:
            ...
            model_state = torch.load(checkpoint_path / "model.pt", map_location=device)
            ...
            # Load lambda scheduler if exists
            lambda_path = checkpoint_path / "lambda_scheduler.pt"
            if lambda_path.exists():
                lambda_scheduler.load_state_dict(torch.load(lambda_path, map_location=device))
    ...
    try:
        for epoch in range(start_epoch, config.training.num_epochs + 1):
            ...
            if is_main_process():
                ...
                lambda_q, lambda_d = lambda_scheduler.get_lambda(global_step)
                logger.info(f"  Lambda Q: {lambda_q:.2e}, Lambda D: {lambda_d:.2e}")
            ...
            if epoch % config.training.save_every_n_epochs == 0:
                save_checkpoint(
                    model, optimizer, scheduler, lambda_scheduler,
        ...
        save_checkpoint(
            model, optimizer, scheduler, lambda_scheduler,
            config.training.num_epochs, global_step, args.output_dir
        )

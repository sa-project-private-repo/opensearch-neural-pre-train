"""
V25 Configuration for XLM-RoBERTa SPLADE with IDF-Aware FLOPS.

Key differences from V24:
- Mandatory IDF weight computation from training corpus
- Korean stopword masking support
- Semantic token ratio monitoring
- Explicit idf_weights_path configuration
"""
...
@dataclass
class V25LossConfig:
    """V25-specific loss configuration with mandatory IDF."""

    # Loss weights (same as V24 defaults)
    ...
    """IDF-aware FLOPS regularization weight."""
    ...
    # IDF configuration (V25 specific)
    use_idf_weighting: bool = True
    """Enable IDF-aware penalty (always True for V25)."""
    ...
    """Extra penalty multiplier for stopwords in FLOPS loss."""


@dataclass
class V25Config(BaseConfig):
    """
    V25 Configuration for XLM-RoBERTa SPLADE with IDF-Aware FLOPS.

    Key improvements over V24:
    - Mandatory IDF weight computation/loading
    - Korean stopword masking
    - Semantic token ratio monitoring during training
    - Proper frequency-based discrimination for 250K vocabulary
    """

    # Override with V25-specific configs
    model: V24ModelConfig = field(default_factory=V24ModelConfig)
    """Model configuration (same as V24)."""

    loss: V25LossConfig = field(default_factory=V25LossConfig)
    """Loss configuration with IDF settings."""
    ...
    def __post_init__(self) -> None:
        ...
        # Convert nested dicts to V25-specific dataclasses
        if isinstance(self.model, dict):
            self.model = V24ModelConfig(**self.model)
        if isinstance(self.data, dict):
            self.data = DataConfig(**self.data)
        if isinstance(self.loss, dict):
            self.loss = V25LossConfig(**self.loss)
    ...
    def get_idf_cache_path(self) -> str:
        ...
        return f"{self.training.output_dir}/idf_weights/xlmr_v25_idf"


def create_default_v25_config(
    train_files: Optional[List[str]] = None,
    val_files: Optional[List[str]] = None,
    output_dir: str = "outputs/train_v25",
    ...
    """
    Create a default V25 configuration.
